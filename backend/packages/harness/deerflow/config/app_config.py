...
class RuntimeConfig(BaseModel):
    """Runtime concurrency and resource configuration."""

    max_concurrent_runs: int = Field(
        default=0,
        ge=0,
        description=(
            "Maximum number of simultaneously executing background agent runs. "
            "0 disables the limit (unbounded concurrency). "
            "When the limit is reached, new run requests receive an immediate 503 "
            "response so clients can retry instead of hanging."
        ),
    )
...
class AppConfig(BaseModel):
    ...
    runtime: RuntimeConfig = Field(default_factory=RuntimeConfig, description="Runtime concurrency and resource configuration")
